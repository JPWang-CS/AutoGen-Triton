"""
Triton-Ascend Reduce Prod

output = product(x, axis=-1)
使用 tl.reduce + 自定义 combine_fn 实现通用归约.
"""
...
def _reduce_prod_kernel(
    ...
    for x_loop in range(x_loops):
        ...
        acc = tl.full([XBLOCK_SUB], 1.0, dtype=tl.float32)
        ...
        for r_loop in range(r_loops):
            ...
            x = tl.load(ptrs, mask=block_mask, other=1.0).to(tl.float32)
...
def reduce_prod(x: torch.Tensor, axis: int = -1) -> torch.Tensor:
    ...
    _reduce_prod_kernel[(num_cores, 1, 1)](
...
def main():
    ...
    tests = [
        ("2D small", (128, 64), torch.float32),
        ("2D mid",   (128, 512), torch.float32),
    ]
    ...
    for name, shape, dtype in tests:
        ...
        passed = torch.allclose(result.cpu(), ref.cpu(), rtol=1e-2, atol=1e-2)
