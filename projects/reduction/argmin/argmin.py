"""
Triton-Ascend Reduce Argmin

output = argmin(x, axis=-1) — 返回最小值索引 (int32)
"""
...
def _reduce_argmin_kernel(
    ...
    for x_loop in range(x_loops):
        ...
        for r_loop in range(r_loops):
            ...
            local_idx = tl.argmin(x, axis=1).to(tl.int32) + col_start

            is_better = local_min < best_val
            best_val = tl.where(is_better, local_min, best_val)
            best_idx = tl.where(is_better, local_idx, best_idx)

        tl.store(out_ptr + row_idx * stride_out_row, best_idx, mask=row_mask)


def reduce_argmin(x: torch.Tensor, axis: int = -1) -> torch.Tensor:
    ...
    _reduce_argmin_kernel[(num_cores, 1, 1)](
