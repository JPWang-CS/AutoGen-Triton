"""
Triton-Ascend 矩阵乘法 (Matmul)

使用 tl.dot 实现分块矩阵乘法，通过 @triton.autotune 自动搜索
最优 BLOCK 配置。累加器使用 float32 精度以保证数值稳定性。

设计要点:
- 2D grid: (M // BLOCK_M, N // BLOCK_N) 个 program
- K 维分块: 沿 K 维循环累加 tl.dot 结果
- @triton.autotune: 自动测试多种 BLOCK 配置
- 支持 float16 输入 + float32 累加
"""
...
def _matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_K)):
        ...
        # 加载 A 的分块: A[offs_m, k*BLOCK_K:(k+1)*BLOCK_K]
        a_ptrs = a_ptr + (offs_m[:, None] * stride_am + offs_k[None, :] * stride_ak)
        a_mask = (offs_m[:, None] < M) & (offs_k[None, :] < K)
        a = tl.load(a_ptrs, mask=a_mask, other=0.0)

        # 加载 B 的分块: B[k*BLOCK_K:(k+1)*BLOCK_K, offs_n]
